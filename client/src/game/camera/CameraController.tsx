...
export function useCameraController(mode: CameraMode, target: React.RefObject<THREE.Group | null>) {
  ...
  useFrame((_, delta) => {
    ...
    const lookAt = lookTarget.current.set(pos.x, CAM_LOOK_HEIGHT, pos.z);
    ...
    const shakeZ = shake > 0 ? (Math.random() - 0.5) * shake * 1.1 : 0;

    switch (mode) {
      case 'firstPerson': {
        camera.position.set(
          pos.x + Math.sin(rot) * 0.5,
          CAM_LOOK_HEIGHT + 0.7,
          pos.z + Math.cos(rot) * 0.5,
        );
        lookAt.set(
      ...
      case 'thirdPerson': {
        camera.position.set(
          pos.x - Math.sin(rot) * CAM_DISTANCE,
          CAM_HEIGHT,
          pos.z - Math.cos(rot) * CAM_DISTANCE,
        );
        break;
      }
      case 'topView': {
